...
export default function NavBar() {
  ...
  return (
    <nav
      className={cn(
        'text-foreground header top-0 left-0 z-40 flex w-full items-center bg-transparent',
        sticky
          ? '!fixed !z-[9999] !bg-white !bg-opacity-80 shadow-sticky backdrop-blur-sm !transition dark:!bg-background dark:!bg-opacity-10'
          : 'static'
      )}
    >
      <div className="relative w-full px-3  mx-auto flex items-center justify-between h-20 overflow-x-hidden">
        <div className="flex items-center xmd:gap-5">
          <div className="flex justify-center items-center gap-2">
                    ...
                    {section.mainItems.map((item) => {
                      if ('titleItems' in item) {
                        return item.titleItems && item.titleItems.length > 0 ? (
                          <Accordion key={item.title} type="single" collapsible className="text-black ">
            ...
            <Link href="/">
              <Image className="h-12 min-w-10" width={97} height={33} src={wavefuelLogo} alt="" />
            </Link>
          </div>
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="hidden xmd:flex gap-3">
            <Image src={github} width={33} height={33} alt="" className="hover:scale-105 ease-in duration-200" />
            <Image src={linkedin} width={33} height={33} alt="" className="hover:scale-105 ease-in duration-200" />
