...
const uniqueSetupItems = Array.from(new Set(SetupItems.map((items) => items.headings))).map((headings) =>
  SetupItems.find((items) => items.headings === headings)
);
...
const SetUp = () => {
  ...
  const filterClick = (headings: string) => {
    ...
    if (true) {
      setSetupItemList(SetupItems.filter((items) => items.headings === headings));
    }
  };
    ...
    <div className="relative w-full text-foreground">
      ...
      <div className="pb-16">
        ...
        <div className="w-full max-w-[80%] pb-3">
          ...
          <span className="text-indigo-500 text-xs font-semibold font-['Inter'] leading-none">source code </span>
          <span className="text-xs font-semibold font-['Inter'] leading-none">or the </span>
          <span className="text-indigo-500 text-xs font-semibold font-['Inter'] leading-none">changelog</span>
